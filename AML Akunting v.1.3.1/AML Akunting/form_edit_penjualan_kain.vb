...
Public Class form_edit_penjualan_kain
    Dim ppn As Double
    Private Sub isi_ppn()
        Try
    ...
    Private Sub form_edit_penjualan_kain_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Call isi_ppn()
    ...
    Private Sub txt_qty1_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles txt_qty1.LostFocus
        ...
        If Not txt_qty1.Text = "" Then
            If Decimal.TryParse(input, number) Then
                ...
                Dim qtyc As String = txt_qty_tersedia1.Text
                Dim qty_d, qtyb_d, qtyc_d As Decimal
                Decimal.TryParse(qty, qty_d)
                Decimal.TryParse(qtyb, qtyb_d)
                Decimal.TryParse(qtyc, qtyc_d)
                If qtyb_d > (qty_d + qtyc_d) Then
                    MsgBox("STOK yang diinput lebih besar dari STOK yang tersedia : " & (qty_d + qtyc_d).ToString("#,##0.00"))
                    txt_qty1.Focus()
                End If
            Else
    ...
    Private Sub txt_qty2_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles txt_qty2.LostFocus
        ...
        If Not txt_qty2.Text = "" Then
            If Decimal.TryParse(input, number) Then
                ...
                Dim qtyc As String = txt_qty_tersedia2.Text
                Dim qty_d, qtyb_d, qtyc_d As Decimal
                Decimal.TryParse(qty, qty_d)
                Decimal.TryParse(qtyb, qtyb_d)
                Decimal.TryParse(qtyc, qtyc_d)
    ...
    Private Sub txt_qty3_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles txt_qty3.LostFocus
        ...
        If Not txt_qty3.Text = "" Then
            If Decimal.TryParse(input, number) Then
                ...
                Dim qtyc As String = txt_qty_tersedia3.Text
                Dim qty_d, qtyb_d, qtyc_d As Decimal
                Decimal.TryParse(qty, qty_d)
                Decimal.TryParse(qtyb, qtyb_d)
                Decimal.TryParse(qtyc, qtyc_d)
    ...
    Private Sub txt_harga_jual_ppn_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles txt_harga_jual_ppn.LostFocus
        ...
        If Not txt_harga_jual_ppn.Text = "" Then
            If Decimal.TryParse(input, number) Then
                txt_harga_jual_ppn.Text = number.ToString("#,##0.00")
            Else
                MessageBox.Show("Input harus berupa angka dengan format yang benar", "Kesalahan", MessageBoxButtons.OK, MessageBoxIcon.Error)
    ...
    Private Sub hitungstok()
        ...
        'txt_qty.Text = qty_d.ToString("#,##0.00########")
    ...
    End Sub
    Private Sub hitungomsetlama()
        'Dim omset, dppomset, totalkain, dppkain, polos, totalkainasal, totalhargaomset As Decimal
        'Decimal.TryParse(txt_sjx_grand_total_asal.Text, omset)
        'Decimal.TryParse(txt_dpp_sjx_asal.Text, dppomset)
        'Decimal.TryParse(txt_grand_total_kain_asal.Text, totalkainasal)
    ...
    Private Sub btn_simpan_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_simpan.Click
        ...
        Else
            ...
            If txt_grand_total_kain_asal.Text <> "" And txt_dpp_sjx_asal.Text <> "" Then
                Call updateomset()
            End If
            MsgBox("DATA PENJUALAN KAIN Berhasil Di UPDATE")
            form_penjualan.Show()
    ...
    Private Sub updategrey1()
        ...
        Using cony As New MySqlConnection(sLocalConn)
            ...
            End Using
        End Using
        Dim qty, qtyasal, dpp_jual As Decimal
        Decimal.TryParse(txt_qty1.Text, qty)
        Decimal.TryParse(txt_qty_asal1.Text, qtyasal)

        stok_keluar = stok_keluar - qtyasal + qty
        stok_akhir = stok_akhir + qtyasal - qty
    ...
    Private Sub updategrey2()
        ...
        Decimal.TryParse(txt_qty_asal2.Text, qtyasal)
    ...
    Private Sub updategrey3()
        ...
        Decimal.TryParse(txt_qty_asal3.Text, qtyasal)
    ...
    End Sub
    Public Sub LoadIdGreyByKodeLama(ByVal kode As String)
        Try
            ...
            If txt_id_grey1.Text <> 0 Then
                Dim query2 As String = "SELECT stok_akhir, kode_grey FROM tbgrey WHERE id_grey = @id1;"
                Using conn As New MySqlConnection(sLocalConn)
                    Using cmd As New MySqlCommand(query2, conn)
                        ...
                        Using reader As MySqlDataReader = cmd.ExecuteReader()
                            If reader.Read() Then
                                ...
                                Dim qtytersedia As Decimal
                                Decimal.TryParse(reader("stok_akhir"), qtytersedia)
                                txt_qty_tersedia1.Text = qtytersedia.ToString("#,##0.00")
                            End If
                        End Using
                    End Using
                End Using
                    ...
                    Using cmd As New MySqlCommand(query2a, conn)
                        ...
                        Using reader As MySqlDataReader = cmd.ExecuteReader()
                            If reader.Read() Then
                                ...
                                dtp_tanggal_beli1.Text = reader("tanggal").ToString()
                                txt_supplier1.Text = reader("supplier").ToString()
                                Dim harga1, qty1, dpp1, ppn1, total1 As Decimal
                                ...
                                txt_qty_asal1.Text = qty1.ToString("#,##0.00")
                                ...
                                txt_kode_beli1.Text = reader("kode").ToString()
                                txt_no_faktur1.Text = reader("no_faktur").ToString()
                            End If
                        End Using
                    End Using
                End Using
            ...
            If txt_id_grey2.Text <> 0 Then
                Dim query3 As String = "SELECT stok_akhir, kode_grey FROM tbgrey WHERE id_grey = @id2;"
                Using conn As New MySqlConnection(sLocalConn)
                    Using cmd As New MySqlCommand(query3, conn)
                        ...
                        Using reader As MySqlDataReader = cmd.ExecuteReader()
                            If reader.Read() Then
                                ...
                                Dim qtytersedia As Decimal
                                Decimal.TryParse(reader("stok_akhir"), qtytersedia)
                                txt_qty_tersedia2.Text = qtytersedia.ToString("#,##0.00")
                            End If
                        End Using
                    End Using
                End Using
                    ...
                    Using cmd As New MySqlCommand(query3a, conn)
                        ...
                        Using reader As MySqlDataReader = cmd.ExecuteReader()
                            If reader.Read() Then
                                ...
                                dtp_tanggal_beli2.Text = reader("tanggal").ToString()
                                txt_supplier2.Text = reader("supplier").ToString()
                                Dim harga2, qty2, dpp2, ppn2, total2 As Decimal
                                ...
                                txt_qty_asal2.Text = qty2.ToString("#,##0.00")
                                ...
                                txt_kode_beli2.Text = reader("kode").ToString()
                                txt_no_faktur2.Text = reader("no_faktur").ToString()
                            End If
                        End Using
                    End Using
                End Using
            ...
            If txt_id_grey3.Text <> 0 Then
                Dim query4 As String = "SELECT stok_akhir, kode_grey FROM tbgrey WHERE id_grey = @id3;"
                Using conn As New MySqlConnection(sLocalConn)
                    Using cmd As New MySqlCommand(query4, conn)
                        ...
                        Using reader As MySqlDataReader = cmd.ExecuteReader()
                            If reader.Read() Then
                                ...
                                Dim qtytersedia As Decimal
                                Decimal.TryParse(reader("stok_akhir"), qtytersedia)
                                txt_qty_tersedia3.Text = qtytersedia.ToString("#,##0.00")
                            End If
                        End Using
                    End Using
                End Using
                    ...
                    Using cmd As New MySqlCommand(query4a, conn)
                        ...
                        Using reader As MySqlDataReader = cmd.ExecuteReader()
                            If reader.Read() Then
                                ...
                                dtp_tanggal_beli3.Text = reader("tanggal").ToString()
                                txt_supplier3.Text = reader("supplier").ToString()
                                Dim harga3, qty3, dpp3, ppn3, total3 As Decimal
                                ...
                                txt_qty_asal3.Text = qty3.ToString("#,##0.00")
                                ...
                                txt_kode_beli3.Text = reader("kode").ToString()
                                txt_no_faktur3.Text = reader("no_faktur").ToString()
                            End If
                        End Using
                    End Using
                End Using
                ...
                Using cmd As New MySqlCommand(query5, conn)
                    ...
                    Using reader As MySqlDataReader = cmd.ExecuteReader()
                        If reader.Read() Then
                            ...
                            Dim qty, harga, dpp, ppntotal, total, hargadpp As Decimal
                            ...
                            txt_harga_jual_ppn.Text = harga.ToString("#,##0.00")
                            ...
                            hargadpp = harga / (1 + (ppn / 100))
                            txt_harga_jual_dpp.Text = hargadpp.ToString("#,##0.00")
                        End If
                    End Using
                End Using
            End Using
                    ...
                    Using cmd As New MySqlCommand(query6, conn)
                        ...
                        Using reader As MySqlDataReader = cmd.ExecuteReader()
                            If reader.Read() Then
                                ...
                                Dim omset_d, dpp_d, dppjual_d, grand_total_kain_d, polos_d As Decimal
    ...
    Public Sub LoadIdGreyByKode(ByVal kode As String)
        Try
            ...
            If txt_kode_omset.Text <> "" Then
                ...
                Using conn As New MySqlConnection(sLocalConn)
                    Using cmd As New MySqlCommand(query6, conn)
                        ...
                        Using reader As MySqlDataReader = cmd.ExecuteReader()
                            If reader.Read() Then
                                txt_id_omset.Text = reader("id_omset").ToString()
                                dtp_omset.Value = reader("tanggal")
                                txt_sjx_grand_total_asal.Text = reader("omset").ToString()
                                txt_dpp_sjx_asal.Text = reader("dpp").ToString()
                                txt_grand_total_kain_asal.Text = reader("grand_total_kain").ToString()
                                txt_total_dpp_jual_asal.Text = reader("dpp_jual").ToString()
                ...
                Dim sjx, totaljual, dpp1, totalomset, polos1, sisa1 As Decimal
                Decimal.TryParse(txt_sjx_grand_total_asal.Text, sjx)
                Decimal.TryParse(txt_total_harga.Text, totaljual)
                Decimal.TryParse(txt_grand_total_kain_asal.Text, totalomset)
                Decimal.TryParse(txt_polos_asal.Text, polos1)
                Decimal.TryParse(txt_total_dpp_jual_asal.Text, dpp1)

                If txt_status.Text = "Kain" Then
                    totalomset = totalomset - totaljual
                    dpp1 = totalomset / (1 + (ppn / 100))
                    sisa1 = sjx - polos1 - totalomset
                    txt_total_dpp_jual_asal.Text = dpp1
                    txt_grand_total_kain_asal.Text = totalomset
                    txt_polos_asal.Text = polos1
                    txt_sisa_omset_asal.Text = sisa1
                Else
                    polos1 = polos1 - totaljual
                    sisa1 = sjx - polos1 - totalomset
                    txt_total_dpp_jual_asal.Text = dpp1
                    txt_grand_total_kain_asal.Text = totalomset
                    txt_polos_asal.Text = polos1
    ...
    Private Sub txt_status_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txt_status.TextChanged
        If txt_status.Text = "Kain Polos" Then
            btn_hitung.Enabled = False
            btn_hitung_polos.Enabled = True
        ElseIf txt_status.Text = "Kain" Then
            btn_hitung.Enabled = True
            btn_hitung_polos.Enabled = False
        Else
            btn_hitung_polos.Enabled = True
